...
float FMathEx::RandomSameDigitsNumber(float Value)
{
	...
	int32 DecimalDigits = DecimalValue > 0 ? (int)FMath::LogX(10, DecimalValue) + 1 : 1;
	int32 FractionalDigits = FMath::IsNearlyZero(Value - DecimalValue) ? 0 : 2;

	float Result = FMath::RandRange(0, (int32)FMath::Pow(10, DecimalDigits) - 1);
	if (FractionalDigits > 0)
	{
		int32 Divider = (int32)FMath::Pow(10, FractionalDigits);
