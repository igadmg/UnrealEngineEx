...
#define UE_LOG_EX_WCO(CategoryName, Verbosity, WorldContextObject, Format, ...) \
	UE_LOG(CategoryName, Verbosity, TEXT("%s> %s[%s] %s %s: ") Format, *::GetWorldName(WorldContextObject), *::GetNetModeString(WorldContextObject), TEXT_ENUM(::GetNetRole(WorldContextObject)), *WorldContextObject->GetName(), UTF8_TO_TCHAR(__FUNCTION__), ##__VA_ARGS__)
